...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  const statusClass = styles[`status${status.charAt(0).toUpperCase() + status.slice(1).toLowerCase()}`];
...
export const SolicitacaoList: React.FC<SolicitacaoListProps> = ({
  ...
  const colunas: ColumnType<Solicitacao>[] = useMemo(() => [
    { header: 'Especialidade', accessor: 'appointmentTypeName' }, // Adapte ao nome do campo no DTO
    { header: 'Médico', accessor: 'doctorName' },
    { header: 'Data Solicitada', accessor: 'dia' }, // Formatar data se necessário no Table ou aqui
    { 
      header: 'Status', 
      accessor: 'status',
      render: (row: Solicitacao) => <StatusBadge status={row.status} /> // Renderização customizada
    }, 
  ], []);
