...
public class AppointmentConsumerController {
     ...
     */
    @GetMapping
    public ResponseEntity<Page<RequestAppointmentResponseDto>> getAllConsultaRequests(
            @RequestParam(required = false) String search,
            @RequestParam(required = false) String status,
            Pageable pageable
    ) {
        // Passa ambos os filtros para o serviço
        Page<RequestAppointmentResponseDto> requestsPage = adminConsultaService.getAllConsultaRequests(search, status, pageable);
     ...
     */
    @PatchMapping("/{id}")
    public ResponseEntity<RequestAppointmentResponseDto> updateConsulta(
            @PathVariable String id,
            @RequestBody UpdateAppointmentDTO appointmentDTO
    ) {
        // O DTO [cite: 1] e o endpoint  já estavam corretos.
        RequestAppointmentResponseDto updatedDoc = adminConsultaService.updateConsultaStatus(id, appointmentDTO);
